...
Coulomb::Config buildConfig() {
    ...
    // config.species[0].eps = 8.282660e+10;
    // config.species[1].eps = 8.282660e+10;
    ...
    for (int s1 = 0; s1<config.nspecies; s1++) {
        ...
        for (int s2 = 0; s2<config.nspecies; s2++) {
            ...
            // config.species[s1].nu[s2] = coefs_nu(0, 0, &config);
        }
    }
