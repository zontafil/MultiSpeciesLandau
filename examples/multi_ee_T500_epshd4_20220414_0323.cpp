...
Coulomb::Config buildConfig() {
    ...
    config.dt = 1/16.;
    config.n_timesteps = 500; // number of timesteps
    ...
    // config.h = 2.*L/MARKERS_PER_DIM / config.nspecies / 2;
    config.h = 2.*L/MARKERS_PER_DIM;
    // config.h = 2.*L/MARKERS_PER_DIM;
    config.eps = 0.64*pow(config.h / 4, 1.98);
