...
Coulomb::Config buildConfig() {
    ...
    double L = 10;
    int MARKERS_PER_DIM = 12;

    config.dx = 1E-2; // dx for finite difference derivative
    config.dt = 1/16.;
    config.n_timesteps = 110; // number of timesteps
    config.newtonTolerance = 1E-15; // minimum target for error of eq. of motions
    config.useNewton = 0;
    config.maxEOMIterations = 20;
    config.h = 2.*L/MARKERS_PER_DIM;
    config.eps = 0.64*pow(config.h, 1.98);
    // config.eps = 0.64*pow(config.h, 1.98) / 10;
    config.xmin = -L;
    config.xmax = L;
    config.ymin = -L;
    config.ymax = L;
