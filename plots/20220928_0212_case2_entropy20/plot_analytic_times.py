...
def Tisothermal(t, ma, mb, n, Tax, Tay, Tbx, Tby, l00, l01, l11):
    ...
    # tau_a_thermal = time_thermalization(t, ma, mb, n, 6.6, Ta0, Tb0)
    # tau_b_thermal = time_thermalization(t, mb, ma, n, 6.6, Tb0, Ta0)
    
    ret = [0,0,0,0]
    ret[0] = (Ta0+Tb0)/2. + (Ta0-Tb0)/2.* np.exp(-t/tau_a_thermal) + (Tax-Tay)/2 * np.exp(-t/tau_a_iso[0])
    ret[1] = (Ta0+Tb0)/2. + (Ta0-Tb0)/2.* np.exp(-t/tau_a_thermal) + (Tay-Tax)/2 * np.exp(-t/tau_a_iso[1])
    ret[2] = (Ta0+Tb0)/2. + (Tb0-Ta0)/2.* np.exp(-t/tau_b_thermal) + (Tbx-Tby)/2 * np.exp(-t/tau_b_iso[0])
    ret[3] = (Ta0+Tb0)/2. + (Tb0-Ta0)/2.* np.exp(-t/tau_b_thermal) + (Tby-Tbx)/2 * np.exp(-t/tau_b_iso[1])
