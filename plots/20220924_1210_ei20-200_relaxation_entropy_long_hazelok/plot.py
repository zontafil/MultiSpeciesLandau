...
if enableLinePlots:
    ...
    if enableAnalyticFunctions:
        ...
        # times_log[0] = 10**(-7)
        plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
        Tanalytic = Tisothermal(times_log, ma, mb, na, clog00, Tax, Tay, Tbx, Tby)
        for s in range(0, nspecies):
        ...
        legend = plt.legend(bbox_to_anchor=(1,1.15), loc="upper right")
    ...
    if enableAnalyticFunctions:
        plt.clf()
        plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
        Tanalytic = Tisothermal(times, ma, mb, na, clog00, Tax, Tay, Tbx, Tby)
        for s in range(0, nspecies):
            plt.scatter(times, TspeciesAxes[s, 0], label=specieNames[s]+" x", s=0.5)
            plt.plot(times, TspeciesAxes[s, 0])
            plt.scatter(times, TspeciesAxes[s, 1], label=specieNames[s]+" y", s=0.5)
